...
/// Total number of query execution errors (Hyperindex errors)
pub static QUERY_ERRORS: Lazy<Counter> = Lazy::new(|| {
    let counter = Counter::with_opts(
        Opts::new("converter_query_errors_total", "Total number of query execution errors")
            .namespace("converter")
    ).unwrap();
    REGISTRY.register(Box::new(counter.clone())).unwrap();
    counter
